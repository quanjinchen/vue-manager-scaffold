...
function unwrapLoginInfo(loginInfo: LoginInfoResponse) {
  if (
    loginInfo &&
    typeof loginInfo === "object" &&
    loginInfo.data &&
    typeof loginInfo.data === "object"
  ) {
...
function collectPermissions(
  ...
  const loop = (list: Record<string, any>[]) => {
    list.forEach((item) => {
      if (item.menuCode ?? item.permissionCode) {
        permissions.add(String(item.menuCode ?? item.permissionCode));
      }
...
function mapMenuList(menuList: Record<string, any>[] = []): AccessMenuItem[] {
  return menuList
    .filter(
      (item) =>
        item &&
        (item.path || (Array.isArray(item.children) && item.children.length)),
    )
    .map((item) => ({
      ...
      permissions:
        (item.menuCode ?? item.permissionCode)
          ? String(item.menuCode ?? item.permissionCode)
          : undefined,
